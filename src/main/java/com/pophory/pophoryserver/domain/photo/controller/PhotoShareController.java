...
public class PhotoShareController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "공유할 사진 정보 조회 실패"),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "사진 공유 성공"),
            @ApiResponse(responseCode = "400", description = "사진 공유 실패"),
            @ApiResponse(responseCode = "500", description = "서버 오류")
    })
    public ResponseEntity<Void> getSharedPhoto(@PathVariable("photoId") Long sharedPhotoId, Principal principal) {
        Long photoId = photoShareService.addPhotoFromShare(MemberUtil.getMemberId(principal), sharedPhotoId);
        return ResponseEntity.created(PhotoUtil.getURI(photoId)).build();
    }
}
